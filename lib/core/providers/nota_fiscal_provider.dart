...
class NotaFiscalProvider extends ChangeNotifier {
  ...
  static const _kPrefUltimaSincronizacao = 'medvie.nota.ultima_sincronizacao_utc';
  ...
  void _onNotaAtualizada(Map<String, dynamic> json) {
    ...
    final versaoEvento = (json['versao'] as num?)?.toInt();
    final versaoLocal = _notas[index].versao;
    if (versaoEvento != null &&
        versaoLocal != null &&
        versaoEvento <= versaoLocal) {
      return;
    }
    ...
    while (tentativa < _kReconciliacaoTentativasMax) {
      try {
        ...
        for (final dto in atualizacoes) {
          ...
          final versaoLocal = _notas[index].versao;
          if (versaoLocal != null && dto.versao <= versaoLocal) {
