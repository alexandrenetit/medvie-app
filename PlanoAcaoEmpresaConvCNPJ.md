...
- [x] **T8.1** Semantics em toggles/cards/radio; foco visível; contraste (tokens — `textFaint` legível).
- [x] **T8.2** Estados: vazio/foco/válido/erro/loading/sucesso tratados. Helper text abaixo do CTA quando gates não passam (`cnpj-cta-helper`, espelha hint do toggle emitir) + snackbar verde de sucesso em `_finalizarPosSalvar` path "deixar para depois" ("Serviço salvo. Emita a NFS-e depois em Notas."). Loading/erro do sheet cadastro (T3.1) e submit (T6.x) já cobertos. Foco Material+InkWell ripple. Sucesso com emitir já tinha snackbar (F6). 2 testes widget (helper visível inicial + helper visível com tomador no provider mas sem seleção). Gate leve: analyze 0 / 692 pass / 4 golden baseline Windows (sem regressão vs T8.1).
...
| 2026-06-16 | **Pós-F9 gaps fechados (3/4).** Branch `fix/cnpj-pos-f9-gaps` (4 commits, 1 arquivo morto a remover follow-up). Gap #2 `valorInicial` propaga p/ `AtendimentoCnpjFlow` (b843b3c — param `valorInicial` + initState aplica + call site). Gap #4 4 goldens baseline Windows regenerados (b5cbe35 — `BottomNav×2` + `AuthScreen×2`; diff 0.16-0.17% = font rendering, não regressão; suite agora 696/696). Gap #3 modo edição migrado p/ `AtendimentoCnpjFlow` (ded96fa — params `modoEdicao`+`servicoInicial`+`onSalvarEdicao`+`onExcluirOuCancelar` `Future<void>`; `_hidratarEdicao` resolve Tomador em postFrameCallback; UI condicional esconde toggle+preview, label "Salvar alterações", botão excluir/cancelar injetado; call site em `add_servico_modal.dart:516-526` via `else if (widget.modoEdicao)`; legado vira dead code). Gap #1 NBS = 🛈 decisão humana externa (sem auto-resolver; §10 com link p/ validação contador/especialista). Gate 4/4 final: analyze 0; test 696/696; dcm 0; apk 51,4s. | #2 #3 #4 | fix/cnpj-pos-f9-gaps (b843b3c / b5cbe35 / ded96fa) | 4/4 ✓ | 🛈 NBS humano |
...
- 🛈 NBS dos tipos CNPJ atribuídos em `servico.dart:52` (`40119`/`40101`/`40201`) — **decisão humana externa** (consultar contador/especialista fiscal; NBS foi atualizado em 2024/2025 com a reforma tributária, não posso validar a tabela vigente daqui). Código `40119` (catch-all "outros serviços de saúde humana") está sendo usado para 4 tipos distintos (`plantao`/`atoAnestesico`/`procedimentoCirurgico`/`outros`), o que é semanticamente fraco — provável refatoração quando vier a validação. Comentário enganoso "⚠ NBS são placeholders" removido de `atendimento_cnpj_flow.dart:40` e substituído por referência a este §10.
- ✅ Inferência local de retenção isolada (F5.T5.3 + F7.T7.2 + F7.T7.3): `_calcularPreview`/`_buildPreviewFiscal` vivem **só** no branch `modoEdicao` (legado isolado linhas 503-523). `AtendimentoCnpjFlow`/`PreviewFiscalCnpjCard` têm 0 refs a `valorIssRetido`/`valorIrrfRetido`. UI nova 100% livre de inferência local. Remoção total = migração do modo edição para flow novo (escopo de fase futura; UX edição ≠ criação justifica manter legado).
- ⚠ `AddServicoModal.valorInicial` (param) não é propagado para `AtendimentoCnpjFlow` no modo criação. Call site vivo: `simulador_bottom_sheet.dart:253` → preenche valor (`resultado?.valorLiquido`), mas `add_servico_modal.dart:79-83` aplica via `_valorController` do **modal legado** (linha 82: `.format(widget.valorInicial)`); o flow novo (F4) tem `_valorController` próprio e ignora o param. Bug silencioso na UX do simulador → botão "Adicionar" do simulador abre modal sem valor pré-preenchido. Task futura fora do escopo atual (envolve acoplamento `add_servico_modal` ↔ `AtendimentoCnpjFlow`: propagar `valorInicial` no construtor do flow ou no call-site do `add_servico_modal`).

Fechados em pós-F9 (2026-06-16, branch `fix/cnpj-pos-f9-gaps`):
- ✅ Gap #2 — `valorInicial` propagado para `AtendimentoCnpjFlow` (b843b3c): novo param opcional `valorInicial` no flow + aplicação em `initState` (seta `_valor.text`/`_valorAtual`); call site em `add_servico_modal.dart:480` passa o param. Bug silencioso do simulador resolvido.
- ✅ Gap #4 — 4 testes golden baseline Windows regenerados (b5cbe35): `BottomNav×2` + `AuthScreen×2`. Diff 0.16-0.17% = font rendering Windows vs baseline (não regressão). Suite agora 696/696 (era 692/4 golden fail).
- ✅ Gap #3 — modo edição CNPJ migrado para `AtendimentoCnpjFlow` (ded96fa): novo param `modoEdicao` + `servicoInicial` + callbacks `onSalvarEdicao`/`onExcluirOuCancelar` (ambos `Future<void>` para aceitar snackbar/await). Pré-popula tipo/tomador/valor/data/descrição/status/horários em `_hidratarEdicao` (resolve Tomador via `OnboardingProvider` em `addPostFrameCallback`). UI condicional: oculta toggle "emitir agora" + `PreviewFiscalCnpjCard` no modo edição; label CTA "Salvar alterações"; injeta botão "Excluir / Cancelar" (parent decide ação). `_confirmar` constrói `Servico.copyWith` (preserva id/status fiscal) e chama `onSalvarEdicao`. Call site em `add_servico_modal.dart:516-526` injeta `AtendimentoCnpjFlow(modoEdicao: true, ...)` via nova branch `else if (widget.modoEdicao)`. Legado virou dead code (caminho nunca executado); remoção = follow-up.
