...
| 2026-06-16 | **F7 T7.3.** Auditoria `grep` zero-refs externas aos 6 símbolos legados (`_buildDropdownTipo`/`_buildDropdownTomadores`/`_buildSemTomadores`/`_calcularPreview`/`_buildPreviewFiscal`/`_PreviewFiscalCard`): privado Dart = inacessível cross-file; callsites (linhas 485, 490, 491, 566) **dentro do `else` (modo edição)** = vivos pro legado isolado T7.2 → **nenhum removível com segurança** (remover quebra `modoEdicao`). Gap real = state órfão só-criação: `_carregandoSugestao` field + `_carregarSugestao()` (90-107) + `_mapearTipoServico()` (110-120) + call `initState` (77) + spinner `if (_carregandoSugestao)` (build 527-533) + 10 `debugPrint('[SALVAR]...')` em `_salvar` (T0.1 §3.A "_limpar em F7_"). Todos removidos (5 edits, 1 arquivo). `valorInicial` (param) + bloco `if (valorInicial != null)` (79-83) **mantidos** (call site vivo em `simulador_bottom_sheet.dart:253`; gap F4 = `valorInicial` não propagado para `AtendimentoCnpjFlow`, candidato a task futura). Gate leve: analyze 0; 690/687 pass / 4 golden baseline Windows (sem regressão). | T7.3 | feat/cnpj-f7-integracao-entrypoint (T7.3 hash) | leve ✓ | gate pesado F7 → merge develop |
