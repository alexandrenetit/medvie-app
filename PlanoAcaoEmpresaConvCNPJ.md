...
- [ ] **T2.1** Novo `lib/features/syncview/widgets/tomador_selector_sheet.dart`: card resumo (logo/sigla, razão, CNPJ mono, tags retenção) + botão "Trocar".
- [ ] **T2.2** `showModalBottomSheet` (dark, handle, busca por razão/CNPJ, lista rolável `ListView.builder`, empty state, radio seleção). Espelhar v15.
- [ ] **T2.3** Estado vazio (médico sem tomadores) → CTA cadastrar.
