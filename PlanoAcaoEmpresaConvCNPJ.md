...
- [ ] **T7.3** Remover código morto (dropdowns/helpers só-criação) após confirmar sem uso (não remover sem confirmar impacto — CLAUDE.md). **Pré-condição T7.2:** auditar `grep` zero-referências a `_buildDropdownTipo`/`_buildDropdownTomadores`/`_buildSemTomadores`/`_calcularPreview`/`_buildPreviewFiscal`/`_PreviewFiscalCard` (legados) fora do branch `modoEdicao` antes de remover.
